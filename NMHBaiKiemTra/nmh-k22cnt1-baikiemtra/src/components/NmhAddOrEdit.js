...
export default function NmhAddOrEdit({ onNmhClose, onNmhSubmitForm, renderStudent }) {
    ...
    useEffect(() => {
        if (renderStudent) {
            setNmhMaSV(renderStudent.nmhMaSV || "");
            setNmhHoSV(renderStudent.nmhHoSV || "");
            setNmhTenSV(renderStudent.nmhTenSV || "");
            setNmhPhai(renderStudent.nmhPhai || "");
            setNmhNgaySinh(renderStudent.nmhNgaySinh || "");
            setNmhNoiSinh(renderStudent.nmhNoiSinh || "");
            setNmhMaKH(renderStudent.nmhMaKH || "");
            setNmhHocBong(renderStudent.nmhHocBong || "");
            setNmhDiemTrungBinh(renderStudent.nmhDiemTrungBinh || "");
        }
    }, [renderStudent]);
    ...
    return (
        <div className='border'>
            ...
            <form onSubmit={nmhHandleSubmit}>
                ...
                <button className='btn btn-primary' type='submit'>Ghi lại</button>
                <button className='btn btn-danger' type='button' onClick={nmhHandleClose}>Đóng</button>
            </form>
        </div>
    );
}
